---
type: index
---

# Decision Frameworks

Frameworks and mental models for structured decision-making. For Julian's own decision-maker profile, the live decision journal, and the commitment-lock protocol, see [[performance/decision-journal/_index|Decision Journal]] instead.

## Tools (run inside any framework)
- [[belief-assumption-testing|Belief & Assumption Testing]] - Surface load-bearing claims, label Fact / Assumption / Belief, clarify assumptions and challenge beliefs. The input-cleaning step; how the gut earns its veto.

## Frameworks
- [[choosing-a-decision-framework|Choosing a Decision Framework]] - Two-question filter for picking the right framework; ADHD-optimised to avoid analysis paralysis
- [[sharran-simple-decision-framework|Sharran's Simple Decision Framework]] - Four-step real-time framework: understand context, isolate the issue, accept the risk, map next steps
- [[brain-brand-framework|BRAIN / BRAND Framework]] - Pre-commitment evaluation checklist: Benefits, Risks, Alternatives, Need time/Nothing, Decision
- [[seven-step-decision-process|Seven-Step Decision Process]] - Full deliberate process for high-stakes or complex decisions; covers problem definition through review
