...
- [[memory-model-adoption|Memory Model (Adopted)]] - the decision record for the four-pillar memory frame (Capture, Storage, Injection, Recall), what adoption obligates, how the pillars are currently configured here, and the known federation seam. The model itself is theory and lives in `wiki/technology/Memory systems/`.
