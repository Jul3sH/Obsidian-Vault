> **Mirror copy.** Source: `~/.claude/skills/commitment-guard/SKILL.md`
...
- **A gatekeeper, not an analyst.** Julian's failure mode is not deciding, it is *un-deciding* within ~72h, his conviction re-anchoring to the last emotionally warm voice.
- This skill stops the U-turn and enforces the lock. Full model: `wiki/performance/decision-journal/commitment-lock-protocol.md`.
