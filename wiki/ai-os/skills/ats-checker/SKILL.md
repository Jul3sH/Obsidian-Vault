> **Mirror copy.** Source: `~/.claude/skills/ats-checker/SKILL.md`
...
- Audit a resume against a job description to predict ATS ranking. Provides keyword-match scoring, format compliance checking, and a prioritized fix list.

## What This Does

- ATS systems use two scoring mechanisms:
  1. **Keyword matching**: does your resume contain the hard-required skills, technologies, certifications, and role keywords the JD specifies? ATS scores resumes by how many required keywords appear verbatim (or close matches).
  2. **Parseability**: can the ATS parser read your resume without tripping on formatting, graphics, complex tables, or unconventional structure? Unparseable resumes get kicked out before keyword scoring even happens.
- This skill scores both: it extracts must-have keywords from the JD, searches your resume for matches, audits the format for parser-safety, and produces a **scored report with specific, prioritized fixes**.
