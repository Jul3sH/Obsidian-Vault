> **Mirror copy.** Source: `~/.claude/skills/llm-council/SKILL.md`
...
- One AI gives one answer with no way to gauge quality. The council runs the question through 5 independent advisors thinking from different angles, has them peer-review each other, then a chairman synthesizes agreement, disagreement, and a recommendation.
- Adapted from Andrej Karpathy's LLM Council (multiple models, anonymous peer review, chairman synthesis). Here: sub-agents with different thinking lenses instead of different models.
...
- Best where there's genuine uncertainty and the cost of a bad call is high. If the user already knows the answer and wants validation, the council will likely tell them things they don't want to hear. That's the point.

## the five advisors

Thinking styles, not job titles or personas. Chosen to create tension with each other.
...
### step 1: assess stakes, select context tier, then frame the question

- **Read the lessons-learned log first.** Open `lessons-learned.md` in this skill folder (if present) and apply its briefing disciplines - it's the skill's curated memory of past misses, and is not optional on high-stakes runs.
- **Assess the stakes and pick a context tier before gathering anything.** The framed brief is replicated ~11x across the pipeline (5 advisors + 5 reviewers + chairman), so a deep brief costs roughly 10-20x the tokens of a thin one. Match depth to the decision on two axes: reversibility/cost of being wrong, and how confident you are that you know which files hold the load-bearing facts.

| Tier | Use when | Context the council gets |
|------|----------|--------------------------|
| **1 - Quick brief** | Low stakes, reversible | 30s scan, 2-3 obvious files, your synthesis |
| **2 - Curated deep brief** (default when stakes are high) | Being wrong is expensive AND you know which files matter | Read the full relevant set properly; brief carries raw evidence, the nearest past failure, behavioural priors, your flagged hypothesis |
| **3 - Full-corpus access** | High stakes AND you're unsure which files are load-bearing | Give advisors read-access to the file set + an index; let them pull what they need |

- **Curation beats volume - more files is not automatically better.** The right handful beats dumping everything; Tier 3 is expensive insurance for when you cannot curate confidently, not the default.
- **For Tier 2 or 3, state the chosen tier and a one-line reason and get a quick confirmation from the user before spending the tokens.** For Tier 1, just proceed silently.
- When the user says "council this" (or any trigger phrase), do the following before framing:
...
- Use `Glob`/`Read` to gather context **at the depth your chosen tier dictates** - Tier 1 is the 30-second, 2-3-file scan; Tiers 2-3 mean reading the full relevant set properly. On Tier 2-3, deliberately hunt for the **disconfirming precedent** (the most similar thing already tried, and how it went) and any **behavioural priors** about the people involved - these are exactly the load-bearing facts that thin briefs lose.
...
5. **The nearest past failure** - the most similar prior attempt and its outcome, so advisors can stress-test the recommendation against it
6. **Your own working hypothesis, stated separately and labelled as such** - exposed as an explicit target for the Contrarian and First Principles Thinker to attack, never woven into items 1-5 as if it were neutral fact

- Keep items 1-5 free of your opinion. Item 6 is the only place your hypothesis goes - named and exposed, so the council attacks it rather than absorbing it. But DO make sure each advisor has enough context to give a specific, grounded answer rather than generic advice.
...
- Spawn all 5 advisors simultaneously as sub-agents. Each gets: their advisor identity and thinking style (from the descriptions above), the framed question, and a clear instruction: respond independently. Do not hedge. Do not try to be balanced. Lean fully into your assigned perspective. If you see a fatal flaw, say it. If you see massive upside, say it. Your job is to represent your angle as strongly as possible. The synthesis comes later.
...
- Spawn 5 new sub-agents, one for each advisor. Each reviewer sees all 5 anonymized responses and answers four questions:
1. Which response is the strongest and why? (pick one)
2. Which response has the biggest blind spot and what is it?
3. What did ALL responses miss that the council should consider?
4. Does the strongest response's recommendation fix the *root cause*, or just a symptom? If the brief names a past attempt that failed, would this recommendation end the same way?
...
4. Does the strongest recommendation fix the root cause, or just a symptom? If the brief names a past attempt that failed, would this recommendation end the same way?
...
- The chairman's job is to produce the final council output. It follows this structure, **COUNCIL VERDICT**:
...
4. **Root-cause & precedent check** - before recommending, verify two things. (a) Does the leading recommendation actually resolve the core uncertainty named in the framing? Trace it. (b) Assume it is enacted and fails: against the nearest past failure in the brief, explain why it does not end the same way. If it fails either test, say so and revise the recommendation rather than ship it.
5. **The recommendation** - a clear, actionable recommendation. Not "it depends." Not "consider both sides." A real answer. The chairman can disagree with the majority if the reasoning supports it.
...
## Root-Cause & Precedent Check
[(a) Does the leading recommendation resolve the core uncertainty named in the framing? Trace it. (b) Assume it is enacted and fails — against the nearest past failure in the brief, explain why it does not end the same way. If it fails either test, revise the recommendation rather than ship it.]
...
- If saving, write to `council-transcript-[timestamp].md` in the project's `active/` directory.
...
- **The verdict presentation matters.** Most readers scan the verdict, not the full transcript. Keep the chat verdict clean and scannable (it is presented in chat as markdown, never as an HTML report - see step 5).
