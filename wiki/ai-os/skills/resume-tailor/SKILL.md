> **Mirror copy.** Source: `~/.claude/skills/resume-tailor/SKILL.md`
...
- **User-facing labelling (mandatory):** "Input 1/2/3" is *internal* shorthand only. **Never show it to the user.** In all user-facing output, use the plain names: **"master résumé"** (Input 3), **"template"** (Input 1), **"job description"** (Input 2). E.g. write `Evidence basis (from master résumé): "<quote>"`, not `[Input 3]`. This overrides the `[Input N]` shorthand still used inside the reference files.
- **If Input 1 is not yet chosen**, the user may provide a folder containing multiple resume templates from any supported storage location (Google Drive, Dropbox, OneDrive, local filesystem, or uploaded files). Run the Pre-Section (Template Selection) first to identify the best starting template. See `references/pre-section-template-selection.md`.
