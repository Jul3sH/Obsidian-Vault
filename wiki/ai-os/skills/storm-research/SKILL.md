...
3. Write the report **inside the wiki**, into the folder matching its subject: `wiki/<topic-area>/{topic-slug}-briefing.html` (e.g. `wiki/enterprise-architecture/` for EA/cost topics, `wiki/technology/` for tool/vendor topics). Pick the topic folder that fits the wiki taxonomy; if genuinely unclear, use the `wiki/` root. **Do NOT create a root-level `storm-reports/` folder** - outputs live inside `wiki/`. After writing, add a one-line link to the report in that section's `_index.md`.
...
1. Final deliverable: `wiki/<topic-area>/{topic-slug}-briefing.html` (the v2, post-verification version), filed in the matching wiki topic folder with an `_index.md` entry added - never a root-level `storm-reports/` folder.
