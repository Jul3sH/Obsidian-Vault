...
description: Design a custom Claude Code memory system through a guided interview, then build it. Use when the user wants to design, set up, or evolve a persistent memory system for Claude Code - including first-time setup, adding memory layers (identity, critical context, working memory, long-term knowledge, episodic, decay, promotion), choosing or migrating a storage backend (Obsidian, markdown, SQLite, Supabase, Turso, ChromaDB, Pinecone), tuning decay or promotion rules, or explaining memory paradigms. Triggers - "/memory-architect", "design my memory system", "set up Claude memory", "add a memory layer", "explain how memory decay works".
---

# Memory Architect

> **Provenance:** third-party skill by **Mark Kashef**, saved to this environment 2026-08-01. Not authored here.
>
> **Two known issues in this copy, both flagged rather than silently worked around:**
> 1. **Diagrams reconstructed.** The source received had UTF-8 mojibake (every box-drawing character arrived as `â`). The ASCII diagrams below were rebuilt to be valid and readable; exact character widths may differ from Kashef's originals. The skill states its diagrams are "carefully designed", so replace this file wholesale if a clean copy becomes available.
> 2. **`references/` files are missing.** The skill reads `references/paradigms.md`, `references/infrastructure_options.md` and `references/repo_patterns.md`. None were supplied. Phase 2 and Round 3 will not work as written until they exist.
