# Knowledge Base Index

> *A personal wiki organised around key life domains, with an AI operating layer that runs the processes.*

---

## Planning Layer

- [[projects/_index|Projects]] — All active Projects (OKRs) across workstreams, WSJF-ranked. Use `/project-planner` to add.
- [[deliverables/_index|Deliverables]] — Backlog of all stories, enablers, and generic items across Projects. Ideas linked to a Project live on that Project's own page under its `## Funnel` section; standalone ideas become XXS Project candidates in [[projects/funnel|Project Funnel]].

## AI OS

- [[ai-os/_index|AI OS]] — The operating layer: skills, service design (workflows, ceremonies), system design (tools, memory, infrastructure), and operations logs

## Workstreams (6)

- [[career/_index|Career]] — Professional progression, workplace navigation, interviews, and career strategy; includes [[career/tti/_index|TTI]] engagement context *(OKRs)*
- [[finance/_index|Finance]] — Personal finances: budgeting, savings, and investments *(SMART Goals)*
- [[performance/_index|Performance]] — Personal effectiveness, productivity, self-management, and working with others and with GenAI; includes the Personal MBA framework *(OKRs)*
- [[personal/_index|Personal]] — Hobbies, enjoyment, and life goals *(SMART Goals)*
- [[relationships/_index|Relationships]] — Personal relationships: communication, conflict, and connection *(SMART Goals)*
- [[wellbeing/_index|Wellbeing]] — Physical and mental state: sleep, alcohol, energy, and recovery *(SMART Goals)*

## Reference Libraries

- [[technology/_index|Technology]] — Technical reference library: tools, products, APIs, and vendor catalogues; includes [[technology/sales-marketing-tech/_index|Sales & Marketing Technology]], [[technology/claude-anthropic/_index|Claude & Anthropic]], [[technology/agentic-os/_index|Agentic OS]], and the AI memory architecture comparisons ([[wiki-vs-openbrain|write-time vs query-time]])
- [[enterprise-architecture/_index|Enterprise Architecture]] — EA frameworks, governance, service design, organisational patterns, and market intelligence; includes [[enterprise-architecture/ai-strategy/_index|AI Strategy & Organisation]]

---

*Last updated: 2026-05-27*
