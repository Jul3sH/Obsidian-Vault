...
- Every turn, the Claude-SDK provider tries to **resume** that stored session by passing it to the `claude` binary. The actual conversation lives in Claude Code's own store at `~/.claude/projects/-Users-julianhart-claudeclaw-os/<session-id>.jsonl` (path derives from the checkout folder name - if the folder is ever renamed, sessions move with it).
...
## Update procedure (avoid data loss)

**The official instructions ("delete the old folder, clone a fresh copy") delete `.env` and `store/claudeclaw.db` if followed literally** - neither is in git, so a fresh clone never brings them back. Rename instead of deleting, so there's time to copy that state across:

1. Confirm what's actually running: `launchctl list | grep claw` for the PID, then `lsof -p <PID> -a -d cwd` for its real working directory. Don't assume it matches this doc.
2. Stop: `launchctl bootout gui/$(id -u)/com.claudeclaw.app`
3. Rename (don't delete): `mv ~/claudeclaw-os ~/claudeclaw-os.old`
4. Re-clone via the token link into `~/claudeclaw-os`
5. Restore state: copy `.env` and `store/` from `~/claudeclaw-os.old` into the new `~/claudeclaw-os`
6. Build: `npm install && npm run build`
7. Migrate: `npm run migrate` (the app also migrates on boot)
8. Restart: `launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.claudeclaw.app.plist`
9. Verify version + bot online, then delete `~/claudeclaw-os.old`

**If `.env` is already gone** (this happened 11 Aug 2026 after following the official steps as written): a lost `.env` only loses the *token value*, not the Telegram bot itself. Re-fetch the same bot's token from BotFather (`/mybots` -> API Token) rather than creating a new bot with `/newbot`.
...
- **The official update instructions delete `.env` and `store/` if followed literally** - rename the old folder instead of deleting it, and copy both across before removing the backup.
- **A lost `.env` does not require a new Telegram bot** - re-fetch the existing bot's token from BotFather instead of running `/newbot`.
