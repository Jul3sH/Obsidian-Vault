...
**Gen AI leverage inside existing workflows.** From the outside I can see many pockets of AI initiative across TTI. What I can't tell is whether they add up to an enterprise-wide effort to integrate Gen AI into workflows, govern it, and distribute what works so the whole organisation gains - and that distribution is reuse, which is what enterprise architecture is for. It's also one of the faster paybacks here: the hyperscalers and technology firms are enabling their people at a pace far ahead of most enterprises, and this is how that gap closes. 

The measure follows the same rule as everything else in this document: each workflow taken through the pattern gets a before-and-after measured on its own, so the returns are counted as they land, not promised up front. This initiative is aligned with my extensive background in service design.
...
**Where I've done this before.** The closest precedent is BG Group, where I built and chaired a global networks design authority spanning three regions - the standards, the exception process, and a board of regional design-authority peers deciding against one agreed target. Twenty-plus design standards came out of it and design cycle time fell by up to 30%, measured. What I haven't held is a charter to retire business steering committees at group level, which is exactly why month one is execution on one live decision rather than an argument from track record.
