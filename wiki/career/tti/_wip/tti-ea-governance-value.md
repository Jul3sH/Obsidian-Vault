...
**Where TOGAF fits underneath TBM (added 9 Aug).** TBM needs a governed inventory to map spend onto — without knowing what applications exist, what they run on, and who consumes them, cost allocation is guesswork. **TOGAF provides the structural inventory - what exists, how it maps to capabilities; TBM provides the financial transparency layer on top of it.** This is the answer if Ty asks how the two relate, and it's the reason a lone TBM tooling purchase doesn't work without the architecture discipline underneath it.
