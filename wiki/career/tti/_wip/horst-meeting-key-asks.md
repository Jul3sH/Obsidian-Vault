---
type: negotiation-prep
status: live
created: 2026-09-13
---

# Key Asks for Horst Meeting

This file is the negotiation card for the Horst lunch (expected 14 Sep 2026, before the 15 Sep flight), where Horst has said he will discuss the terms of a job. It was created 13 Sep because Julian needs to walk in with a clear, short list of asks, each tied to a known risk from the engagement arc and the UK relocation reopen. Read it before the lunch; the outcome gets logged to [[tti-comms-log]] and feeds [[hk-uturn-reopen-2026-09]].

> ⚠ As of 13 Sep: the UK-move stand-down is active ([[dec-uk-move]] wobble #5). The lunch is negotiation and information-gathering. Do not commit at the table - "let me review the terms in writing" costs nothing with Horst.

## The one-line frame

Returning means unwinding a completed move and pulling Sophia out of a school she loves. That is the honest, non-desperate reason every ask below is reasonable - use it as the frame, not the custody situation.

## Must-haves (the deal fails without these)

| # | Ask | Risk it closes |
|---|-----|----------------|
| 1 | **Written terms with a start date** - role, number, term, start, and who papers it (HR? Ty? Horst's office?). "Done deal" = contract, not handshake. | Every prior TTI signal died between verbal warmth and paper (Kari w/c 28 Jul, Ty "couple of days", the 20 Aug no). Four wobbles failed on exactly this. |
| 2 | **Multi-year security** - target: term covering Sophia's remaining school years (~6); minimum: 3 years guaranteed, or permanent with meaningful notice/severance. NOT a 6-12 month consulting bridge. | The pre-committed rule: only a signal "firm enough to bank Sophia's full HK school year(s)" justifies staying. A short bridge re-runs the whole 2026 arc in 12 months, with the UK option burned. |
| 3 | **Director-level title and mandate.** | TTI APAC has multiple technology directors under Tony; the EA/governance role Ty described needs peer authority. The file's standing line: director-level is structurally non-negotiable - a Principal cannot coordinate directors. |
| 4 | **Compensation anchored at the previously proposed rate** - HK$200k/month (HK$2.34M/yr, set 9 Aug). Do not open below your own prior written number. | Lock-time analysis: HK only beats the UK financially on a well-paid package. Undercutting your own anchor also reads as desperation. |
| 5 | **Sophia's schooling in the package** - HK international school fees; university support as the stretch ask. Natural framing: "I gave up her school place in the UK; I need her schooled properly here." | The two biggest stay-HK financial negatives in [[HK-BRAIND]] (school fees, overseas university rates). Horst has already heard the schooling complication from Julian directly, so the ask is pre-seeded. |

## Strong asks (negotiate, don't die on)

| # | Ask | Risk it closes |
|---|-----|----------------|
| 6 | **Top-down sponsorship made explicit** - how the role is announced, and Horst/Ty alignment on it. Ideally: Horst tells Ty and Tony before it lands sideways on them. Probe gently: "How do you see Ty's team receiving this?" | The 20 Aug no came from a formal IT-leadership and Executive review. A chairman-fiat job over that chain hands Tony and the review chain a grievance on day one. Kari's own words: activities "would need to be sponsored top-down". |
| 7 | **Reporting line** - who Julian reports to. Ideally Horst-adjacent or CFO-line, not into the chain that assessed and declined him. | Same political risk as 6, structural version. |
| 8 | **Start date with an unwind window** - enough runway to reverse the UK arrangements in an orderly way (school notice, Cecil Road stays let, DB flat plan, shipment redirect; helper notice ends ~30 Sep and may need extending). | The 5-15 Sep execution window is mid-flight; a "start Monday" offer forces rushed, expensive unwinding. |
| 9 | **Benefits: family medical + death-in-service life cover.** | The relocation legal block logged this exact trigger: "evaluate death-in-service at any offer" (life cover has been blocked pending employment). |

## The two candid conversations (spoken framing, added 14 Sep)

**1. Security that outlasts his sponsorship** (the "what if you're not there in two years" risk, said politically). Never mention age or retirement; frame it structurally and he hears the rest himself. This converts must-haves 1-2 and strong ask 6 into one honest paragraph, and covers the Stephan-can't-protect-me problem without naming it:

> "Candidly: if this happens, it happens because you decided it should. That's its strength and its weakness. Your executive team already reviewed this and said no once. So my family's security can't rest on any one person's sponsorship, including yours. I need the protection to be structural: written multi-year terms, and a role announced and owned in a way that means it isn't one man's project."

**2. Backed to do the job, not set up for failure** (strong asks 6-7 spoken). Frame as the role's requirement, not a personal grievance. Do not name Tony, do not relitigate the 20 Aug no; reuse Ty's own "organisational timing" words if it comes up:

> "An enterprise architect looks under the covers and reports what he finds upward. That's uncomfortable for the people being looked at, and honestly I suspect it's part of why this didn't get through the normal chain. If I then report into the leaders who assessed and declined this role, it fails slowly and quietly, whatever I do. For it to work, the reporting line and the announcement need to come from the top."

Then the ask-6 probe: "How do you see Ty's and Tony's teams receiving this?" - invites him to solve the politics himself.

## If he plays the closer: "So what do you want?"

One breath, two halves, equally weighted (this is the anti-"me me me" structure):

> "Two things. The terms that make this safe enough for my family to unwind the UK move: the number I already put in writing, a multi-year commitment, and help with schooling. And the set-up that lets me succeed: your mandate made explicit, and a reporting line that isn't into the chain that already said no. The second half matters as much as the first, because the best terms in the world are pointless if the role is set up to fail."

Pairing rule: every personal ask travels with what it buys him - security buys a committed, full-hearted unwind of the UK move; top-down backing buys a role that delivers instead of getting absorbed. His recorded style rewards this register: he values decisiveness and is impatient with indecision, so the candour is the fit signal, not the risk.

## Table manners (what NOT to do)

- **Do not accept at the table.** Warm agreement on direction, yes; commitment, no - written terms first (stand-down + reopen test pending).
- **Do not volunteer the custody situation as the driver.** It reads as desperation and hands over all leverage. The schooling/unwind frame carries every ask.
- **Do not accept a consulting-bridge reframe.** If it drifts to "start with 6 months and see", the answer is the pre-committed rule in your own words: you can only uproot Sophia again for something secure.
- **Nothing Justin-sourced** reaches Stephan or Horst (standing guardrail). The 3a holding lines apply if Stephan volunteers his rationalisations.
- **Get the next step named before leaving:** who sends what, by when. An open-ended "we'll sort it out" is the Kari/Ty pattern repeating.

## Time and Token Log

| Date | Type | Amount | Notes |
|------|------|--------|-------|
| 2026-09-13 | Machine (interactive session) | unmeasured | File drafted from engagement-arc risks; session total logged at session end |
| 2026-09-14 | Machine (interactive session) | unmeasured | Candid-framing sections added (sponsorship-outlasting security, set-up-to-succeed, closer answer); session total logged at session end |
