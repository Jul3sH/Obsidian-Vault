...
| 2026-08-27 | TTI lessons review session: all 13 rows worked through with Julian one by one - rewritten plainer and in his voice, 1 cut, 1 promoted into the new [[mm-stories-arent-evidence]] model (built and indexed in the same session); index two-tier explainers and the misfiled the-human-mind folder fix also done in this sitting | Julian | [minutes TBC] | Final TTI count: 12 evidence rows + 1 new model; set now 39 models |
| 2026-08-27 | **Complete, reviewed.** Final totals | - | Julian 30 min + review session TBC; machine 2,686,264 subagent tokens | Fable main-session usage not metered, additional to the subagent figure |
