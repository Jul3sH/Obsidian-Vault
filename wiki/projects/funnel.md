...
| AI Engineering Concept Map | A breadth-first, market-aligned map of ~8 AI engineering concept areas (context engineering, memory, tools/MCP, evals, orchestration/workflow architectures, spec-driven development, security incl. agent identity) - one bounded survey article per category in wiki/technology/ plus mm-* mental models where judgement is worth compressing - gives Julian the concept-level judgement to plan AI-leveraged work, set direction, and describe capability to stakeholders in market vocabulary. Serves Performance Objective 1 (Leverage AI, Q4) and underpins Objective 2's Key Person of Influence positioning. Breadth cap is structural: no category gets a second pass until all have a first pass. | Q4 2026 planning confirms Objective 1 (Leverage AI, moved to Q4) is being pursued, or a concrete stakeholder conversation needs the vocabulary sooner. Interview was part-complete at parking (payoff/cost drafted, Performance workstream confirmed); resume via /project-planner. | 2026-08-17 | TBD (create on next `/jira-sync`) |
| AI Task Decomposition Playbook | A /decompose-style skill invoked at sprint/project planning that works through which attack shape fits a task (multi-agent, spec-driven, loops/reflection, generate-evaluate, workflow patterns) turns the concept map into applied judgement at the moment of planning; skill form is the adoption forcing-function per feedback-build-dont-adopt. Seed already exists in [[routing-work-to-agents]]. | AI Engineering Concept Map completes its first-pass breadth (taxonomy agreed + all survey articles written) - this project is explicitly sequenced behind it. | 2026-08-17 | TBD (create on next `/jira-sync`) |
| Start-here enforcement hook | A `UserPromptSubmit` hook that greps the prompt for a tight workstream keyword list and injects a one-line pointer to that workstream's `start-here` file would stop high-value prep being missed at exactly the moment it matters. **Evidenced failure, 8 Aug 2026:** [[tti-ea-governance-value]] - the EA-value argument built specifically for Ty conversations and hardened through two Fable passes - was not consulted while drafting [[tti-ty-engagement-proposal]] for Ty. [[tti-start-here]] points at it three times; the index was fine, the agent simply never opened it. The lesson is that **an index only works if something forces it to be read**, and instructions in CLAUDE.md still depend on the agent remembering, whereas a hook is executed by the harness regardless. Secondary finding: only ONE start-here file exists in the vault (TTI's) - [[uk-relocation-decision]] carries ~30 files with no entry point, so the convention itself is under-adopted. | After the TTI Ty engagement concludes. Two things to decide at Review: the keyword list must stay tight (a hook that fires on everything becomes noise the agent skims past, reproducing the failure), and whether to write start-here files for the other large workstreams first, since the hook is only worth building once there is more than one to point at. | 2026-08-08 | TBD (create on next `/jira-sync`) |
