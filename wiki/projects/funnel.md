...
| BAU Kanban / Portfolio structural alignment | The BAU Kanban board is not structurally aligned with Portfolio Projects (no shared project linkage), which matters more now that the 28 Aug prioritisation fix routes ALL fixed-date and non-project tasks through it: aligning the board's structure with the Portfolio layer makes Kanban work traceable to projects and closes the gap flagged in [[prioritization-framework]] (Execution routing, known gap). Ultimately about improving the current system, so it is discretionary development work - WSJF-scored at Review like any other Performance project. | Julian's call - no pre-set condition. Surfaced 28 Aug during the prioritisation quick fix. | 2026-08-28 | TBD (create on next `/jira-sync`) |
...
| Ceremony trigger audit (crons to hooks) | The other ceremony skills claim cron schedules that nothing executes (`/morning` 6 AM, `/standup` 4 PM weekdays - same fiction the retro's Friday cron was until 3 Sep, when it got a real SessionStart hook). Auditing every claimed schedule and every system in [[systems-register]] whose trigger doesn't actually fire, then wiring real hooks (or retiring the claim), turns paper forcing-functions into executed ones. Reference implementation: the retro Sunday hook in vault `.claude/settings.json`. | Next AI-OS service-design session; or immediately if a second ceremony is caught claiming a schedule that never fired | 2026-09-03 | TBD (create on next `/jira-sync`) |
| Memory review & optimisation | Pruning and tightening the ~25 auto-memory files keeps the always-on MEMORY.md index cheap and every line load-bearing: Julian has spotted removable entries (22 Sep), candidates include superseded corrections, stale context (e.g. HK-era environment notes in user.md), and one-liners that name a topic instead of stating the rule. Scope: apply the CLAUDE.md memory rules (delete wrong, merge duplicates, rewrite descriptions as actionable rules), with wiki mirrors + hidden-file-sync updated in the same operation. XXS, fits one sitting under the Admission Fast Lane. | Next AI-OS session, or whenever Julian names the memories he noticed | 2026-09-22 | TBD (create on next `/jira-sync`) |
| Workflow-shapes catalogue (routing detail) | A catalogue of multi-agent orchestration shapes (pipeline, parallel fan-out, adversarial verify, judge panel, loop-until-dry) with when each fits, as a section in [[routing-work-prompts]] under the several-agents verdict, would make the wide end of routing's width dial a menu instead of an improvisation. Boundary held: how many minds and how they relate = routing; how each behaves once placed = steering. | Julian returns to it after mm-work-types and the genai-task-workflow-log are built and in use | 2026-09-03 | TBD (create on next `/jira-sync`) |
