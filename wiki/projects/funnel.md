...
| AI Engineering Concept Map | A breadth-first, market-aligned map of ~8 AI engineering concept areas (context engineering, memory, tools/MCP, evals, orchestration/workflow architectures, spec-driven development, security incl. agent identity) - one bounded survey article per category in wiki/technology/ plus mm-* mental models where judgement is worth compressing - gives Julian the concept-level judgement to plan AI-leveraged work, set direction, and describe capability to stakeholders in market vocabulary. Near-vehicle for Performance Q4 Obj 3 KR3 (Nate B. Jones 7-skills rubric) and foundation for the Claude cert KR. Breadth cap is structural: no category gets a second pass until all have a first pass. | Q4 2026 planning confirms Objective 3 (AI Agent Builder) is being pursued, or a concrete stakeholder conversation needs the vocabulary sooner. Interview was part-complete at parking (payoff/cost drafted, Performance workstream confirmed); resume via /project-planner. | 2026-08-17 | TBD (create on next `/jira-sync`) |
