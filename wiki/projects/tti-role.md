...
4. **Optional, zero-risk:** add a bridge sentence after the lever table making the mechanism/outcome split explicit ("These are mechanisms, not promised savings. What they're worth to TTI is what month one measures against each one."). Closes the Perplexity critique's one landing hit without new commitment. Requires the three-file sync check.
5. **Ask Stephan on Monday whether Ty can actually decide fast** - before the call, not after. If no, the proposal's job changes from closing to setting a dated next step.
6. **Chase Kari.** Still unanswered since 7 Aug 15:23. He covers the right-to-work question and a candidate stuck cross-region decision for Month 1.
7. **Sequence Tony FIRST** - he must hear it from Ty, framed as pooling authority not confiscating, before it reaches any sign-off desk.
8. **Re-check the mid-August drop-dead before relying on it again.**
