...
- **wiki/ai-os/skills/whatsapp-someday/** - Processes a new WhatsApp "Someday" self-chat export dropped into `~/Library/CloudStorage/Dropbox/5-Performance/Whatsapp someday/`: substantive personal notes routed to their existing wiki home (e.g. Dad's health, a named behavioural-pattern article), share links fetched (Instagram via the `embed/captioned/` URL suffix, which bypasses its bot-wall) and categorised into `raw/brain-dump.md`. Triggers: `/whatsapp-someday` on demand, or automatically weekly via a local launchd job (`com.julianhart.whatsapp-someday`, see `wiki/ai-os/system-design/claude/local-automation.md`) since neither of Claude Code's built-in schedulers (`CronCreate` = session-only, `RemoteTrigger`/`/schedule` = cloud-only, no local filesystem access) can reach the local Dropbox-synced folder.
