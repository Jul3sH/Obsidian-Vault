...
- **wiki/ai-os/skills/notebooklm-py/** - Automate Google NotebookLM via the unofficial `notebooklm-py` Python package. Creates notebooks, ingests **wiki content** as sources, generates flashcards, quizzes, podcasts, mind maps, and reports. Primary use case: Personal MBA wiki topic to flashcard deck. Triggers: `/notebooklm`, "make flashcards from [topic]", "create a notebook about [topic]", "quiz me on [topic]". For **YouTube channels** use `youtube-notebook` instead.
- **wiki/ai-os/skills/youtube-notebook/** - Ingests a **YouTube channel** into NotebookLM, restricted by month, date range, topic, or a combination. Enumeration completeness is proven two independent ways (per-tab metadata walk + uploads-playlist cross-check) before anything is created. Source titles are date-prefixed, and each video gets one *native* NotebookLM briefing doc whose title matches its source so the two can be correlated. Ends in a four-way reconciliation. Triggers: `/youtube-notebook`, "create a notebook of [channel]'s videos from [month]", "all [topic] videos from [channel]".
- **wiki/ai-os/skills/commitment-guard/** - Instruction-only (no scripts). Defends decisions Julian has LOCKED against emotional U-turns and enforces the lock checklist at commit. Triggers on *reopening* a committed decision (not the word "decision"). Implements the F-N-M-T reopen test + 48h stand-down from `wiki/performance/decision-journal/commitment-lock-protocol.md`. Paired with the `feedback-decision-reopening` memory.
- **wiki/ai-os/skills/memory-architect/** - **Third-party skill by Mark Kashef, not authored here.** Guided interview that designs and builds a custom Claude Code memory system (identity, critical context, working memory, long-term knowledge, episodic, decay, promotion layers) over a chosen backend, then hands hook-wiring to `@claude-code-guide`. Triggers: `/memory-architect`, "design my memory system", "add a memory layer". **Incomplete as saved:** its `references/paradigms.md`, `references/infrastructure_options.md` and `references/repo_patterns.md` were not supplied, so Round 3 and Phase 2 will not run as written. Conceptual overlap with the four-pillar model in `wiki/technology/Memory systems/`.
- **wiki/ai-os/skills/prompt-zero/** - Instruction-only (no scripts). Interviews Julian through the **Seven Questions** (accomplish / why it matters / done / wrong / unwritten context / pieces / hard part) so the grounding brief is written in his own words before a significant piece of work starts, and writes it as a `## Prompt Zero` section in the deliverable file. It never drafts his answers. The gate is `AGENTS.md` Deliverable-First **Step 2** (every deliverable; waiver only at `size: 1`, recorded with a date); the backstop is `/standup` Step 3.5. Triggers: `/prompt-zero`, "prompt zero", any deliverable starting without the section, or work that has drifted off-brief.
...
- **wiki/ai-os/skills/behaviour-check/** - Instruction-only (no scripts). Dispatcher over `wiki/performance/biases-index.md`: when a conversation enters a situation where a documented bias fires (twelve trigger situations: weighing options, post-lock wobble, scoping analysis, send/commit point, interpreting silence, financial modelling, reviewing lean-confirming AI analysis, system-building, avoidance rationalisation, certainty spikes, sunk-spend holds, learning choices), it reads the index and raises the matching bias card briefly, then continues. Defers to `commitment-guard` (locked decisions) and `decision-visualisation-check` (option-by-feel). Built 19 Sep 2026 by the bias-history-review deliverable. Maintenance rule: new index row = check this skill's trigger coverage in the same operation.
- **wiki/ai-os/skills/whatsapp-someday/** - Processes a new WhatsApp "Someday" self-chat export dropped into `~/Library/CloudStorage/Dropbox/5-Performance/Whatsapp someday/`: substantive personal notes routed to their existing wiki home (e.g. Dad's health, a named behavioural-pattern article), share links fetched (Instagram via the `embed/captioned/` URL suffix, which bypasses its bot-wall) and categorised into `raw/brain-dump.md`. Trigger: `/whatsapp-someday`, manual only. A weekly local launchd automation was built and tested 2026-08-15 but discontinued the same day (notification delivery from a headless process proved more complex than it was worth) - see `wiki/ai-os/system-design/claude/local-automation.md` for the record.
